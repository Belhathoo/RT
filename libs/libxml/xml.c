...
t_xml		*xml_init(const char *chemin)
{
	...
	if ((x->fd = open(chemin, O_RDONLY)) == -1)
		xml_exit(x, "--", ": no such file", EXIT_FAILURE);
	if (read(x->fd, buf, 0) < 0)
		xml_exit(x, "--", ": must be a file", EXIT_FAILURE);
	if (get_next_line(x->fd, &buf) == 0)
		xml_exit(x, "--", ": empty file", EXIT_FAILURE);
	if (ft_strncmp(buf, "<!DOCTYPE xml>", 14) != 0)
		xml_exit(x, "--", ": document type <!DOCTYPE xml>", EXIT_FAILURE);
	free(buf);
